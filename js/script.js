...
// (Optional) Countdown timer variables if using a countdown feature
let countdownTime = 5;
 ...
 * It uses the same dynamic window as the bottom tick scale.
 * Tick marks and labels are drawn along the container height.
 * The red marker is positioned using the rocket-wrapper's offsetTop (relative to the container).
 */
function updateVerticalTicker() {
  ...
  // Use offsetTop of rocket-wrapper to position the red marker at its center Y.
  const rocketWrapper = document.getElementById("rocket-wrapper");
 ...
 * Before the discount reaches 1%, the rocket moves using dynamic window mapping.
 * Once the discount is ≥ 1%, the rocket remains fixed at the center.
 */
...
/**
 * Update the real-time discount display (above the rocket) and the current run box (bottom right).
 */
